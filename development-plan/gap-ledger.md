...
| BEAM construct | On Silica | Silica today | Class | Closed by | Target |
| --- | --- | --- | --- | --- | --- |
| `gen_tcp`, `gen_udp`, `inet` | OTP's modules, compiled, over the BEES `prim_inet` layer, which sits on Silica's own TCP/IP implementation | Silica ROADMAP chunk 10; spec §20.4 sketches it | C (compiled OTP), B; U (S-22); E until S-22 | A5, I3 | 1.0 |
...
| **S-3** | Monotonic clock and timer primitives. There is no `sleep`: an actor waits by ending its dispatch until a timer's message arrives. | Every timeout depends on them. | Nothing hard | Native `clock_gettime` and a bounded `poll` | **Spec (2026-09-15):** `monotonic_time`, `current_time`, `send_after`, `cancel_timer` (§22.14) and `timer_ref` (§4.5.2); Silica ROADMAP chunk 12. Not implemented. |
...
| **S-27** | **Silica's bare-metal AArch64 path.** | Release 1.0 includes raw AArch64 as a platform. Silica has emitters only for Apple Silicon, Linux AArch64 and ESP32-S3; bare-metal AArch64 is a "later path" in Silica's ROADMAP. | Release 1.0 item 8 (raw AArch64) | ESP32-S3 is the only raw target | **Planned (2026-09-15):** the Silica ROADMAP lists bare-metal AArch64 as the next bare-metal path. Its first board pack is QEMU `virt`, and its first fixed point has the behaviours of ESP32-S3's. `porting_for_os_free_targets.md` §11 now matches that order. No emitter yet. |
...
| **S-30** | **Supervisor children in a `child_spec`.** A `child_spec` has only `behavior: fn(...)`, so a supervisor cannot declare a child that is itself a supervisor. §15.4.13.2 says a supervisor that breaches its restart intensity escalates "via the link established at its spawn", but no spawn path creates that link. | BEES's own supervisors sit inside the programmer's tree, and position blocks (D22) and the per-core spawn supervisors assume supervisors started by supervisors. `supervisor` modules routinely have supervisor children. | A2, A4 | Supervisors started with `spawn_registered_supervisor` as extra roots, which loses escalation | Spec §15.4.13.2 (`child_spec`); found while writing the 2026-09-15 edits. **Not yet specified.** |
