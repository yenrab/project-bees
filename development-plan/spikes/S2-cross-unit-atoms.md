...
**Status:** run 2026-09-17 on `silica-999977-macos-applesilicon`. **Failed.** S-1 is confirmed as a
hard blocker (gap ledger §3).
...
## 4. Interim, if BEES is not to wait for S-1
...
The recorded golden captures **today's wrong output**, so `make integrate` will flag the day S-1
(or the prelude) changes it.
