...
#
# BEES copy of silica/project_makefiles/topo_silica_config.sh, taking the directory to scan as
# an argument. lib/ units (symlinks to src/) are followed; the paths printed are relative to DIR.
#
# Usage:
#   topo_silica_config.sh DIR [--list-units]
#   topo_silica_config.sh DIR             # write ordered paths to stdout
set -euo pipefail

ROOT="$(cd "${1:?directory}" && pwd)"
shift
cd "$ROOT"
...
list_units() {
  find -L . \( -path './_wd_probe' -o -path './.git' -o -path './.fail' \) -prune -o -type f -name '*.silica' -print \
    | sed 's|^\./||' \
    | LC_ALL=C sort -u
}
